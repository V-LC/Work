# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'ad_delete.ui'
#
# Created by: PyQt5 UI code generator 5.15.1
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_Form(object):
    def setupUi(self, Form):
        Form.setObjectName("Form")
        Form.resize(524, 541)
        Form.setMinimumSize(QtCore.QSize(524, 541))
        Form.setMaximumSize(QtCore.QSize(524, 541))
        Form.setStyleSheet("")
        self.groupBox = QtWidgets.QGroupBox(Form)
        self.groupBox.setGeometry(QtCore.QRect(0, 0, 521, 541))
        self.groupBox.setStyleSheet("QGroupBox {\n"
"border:1px solid rgb(217, 217, 217);\n"
"}")
        self.groupBox.setObjectName("groupBox")
        self.label = QtWidgets.QLabel(self.groupBox)
        self.label.setGeometry(QtCore.QRect(70, 30, 401, 41))
        font = QtGui.QFont()
        font.setFamily("华文行楷")
        font.setPointSize(12)
        self.label.setFont(font)
        self.label.setObjectName("label")
        self.btn_check = QtWidgets.QPushButton(self.groupBox)
        self.btn_check.setGeometry(QtCore.QRect(130, 470, 93, 28))
        self.btn_check.setStyleSheet("background-color: rgb(194, 170, 255,200);\n"
"")
        self.btn_check.setObjectName("btn_check")
        self.btn_delete = QtWidgets.QPushButton(self.groupBox)
        self.btn_delete.setGeometry(QtCore.QRect(300, 470, 93, 28))
        self.btn_delete.setStyleSheet("background-color: rgb(194, 170, 255,200);\n"
"")
        self.btn_delete.setObjectName("btn_delete")
        self.layoutWidget = QtWidgets.QWidget(self.groupBox)
        self.layoutWidget.setGeometry(QtCore.QRect(40, 70, 451, 81))
        self.layoutWidget.setObjectName("layoutWidget")
        self.gridLayout = QtWidgets.QGridLayout(self.layoutWidget)
        self.gridLayout.setContentsMargins(0, 0, 0, 0)
        self.gridLayout.setObjectName("gridLayout")
        self.label_2 = QtWidgets.QLabel(self.layoutWidget)
        self.label_2.setObjectName("label_2")
        self.gridLayout.addWidget(self.label_2, 0, 0, 1, 1)
        self.lineEdit_id = QtWidgets.QLineEdit(self.layoutWidget)
        self.lineEdit_id.setStyleSheet("background-color: rgb(255, 255, 255,120);")
        self.lineEdit_id.setObjectName("lineEdit_id")
        self.gridLayout.addWidget(self.lineEdit_id, 0, 1, 1, 1)
        self.label_3 = QtWidgets.QLabel(self.layoutWidget)
        self.label_3.setObjectName("label_3")
        self.gridLayout.addWidget(self.label_3, 1, 0, 1, 1)
        self.lineEdit_psw = QtWidgets.QLineEdit(self.layoutWidget)
        self.lineEdit_psw.setStyleSheet("background-color: rgb(255, 255, 255,120);")
        self.lineEdit_psw.setObjectName("lineEdit_psw")
        self.gridLayout.addWidget(self.lineEdit_psw, 1, 1, 1, 1)
        self.tableWidget = QtWidgets.QTableWidget(self.groupBox)
        self.tableWidget.setGeometry(QtCore.QRect(40, 170, 451, 291))
        self.tableWidget.setStyleSheet("background-color: rgb(255, 255, 255,60);")
        self.tableWidget.setObjectName("tableWidget")
        self.tableWidget.setColumnCount(0)
        self.tableWidget.setRowCount(0)
        self.label_2.setBuddy(self.lineEdit_id)
        self.label_3.setBuddy(self.lineEdit_psw)

        self.retranslateUi(Form)
        QtCore.QMetaObject.connectSlotsByName(Form)
        Form.setTabOrder(self.lineEdit_id, self.lineEdit_psw)
        Form.setTabOrder(self.lineEdit_psw, self.tableWidget)
        Form.setTabOrder(self.tableWidget, self.btn_check)
        Form.setTabOrder(self.btn_check, self.btn_delete)

    def retranslateUi(self, Form):
        _translate = QtCore.QCoreApplication.translate
        Form.setWindowTitle(_translate("Form", "Form"))
        self.groupBox.setTitle(_translate("Form", "删除信息"))
        self.label.setText(_translate("Form", "请输入您想要删除管理员信息的账号以及密码"))
        self.btn_check.setText(_translate("Form", "查询"))
        self.btn_delete.setText(_translate("Form", "删除"))
        self.label_2.setText(_translate("Form", "<html><head/><body><p align=\"right\">账 号：</p></body></html>"))
        self.lineEdit_id.setPlaceholderText(_translate("Form", "请输入10位英文和数字结合的账号"))
        self.label_3.setText(_translate("Form", "<html><head/><body><p align=\"right\">密 码：</p></body></html>"))
        self.lineEdit_psw.setPlaceholderText(_translate("Form", "请输入16位以内英文和数字结合的密码"))
